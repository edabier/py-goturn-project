...
class GOTURN:
    ...
    def __init__(self, root_dir, model_path, device, processed):
        if processed:
            print("Running Goturn with processed images")
        else:
            print("Running Goturn with normal images")
        ...
        if processed :
            frames = os.listdir(root_dir + '/processed_img')
            frames = [root_dir + "/processed_img/" + frame for frame in frames]
        else:
...
if __name__ == "__main__":
    ...
    tester = GOTURN(args.data_directory, args.model_weights, device, False)
    tester.test()
